...
def dice_val_VOI(y_pred, y_true):
    ...
    VOI_lbls = [0, 1, 3, 4, 6, 7, 8, 10, 11, 12, 13, 14, 16, 18, 19, 24, 25, 26, 28, 29, 30, 31, 33, 34, 
                35, 36, 41, 42, 43, 44, 45, 46, 49]
...
def upsample3d_flow_as(inputs, target_as, mode="trilinear", if_rate=False):
    ...
    grid = F.affine_grid(torch.eye(3, 4).unsqueeze(0).expand(inputs.size(0), -1, -1).to(inputs.device), [1, 1, d, h, w])

    res = F.grid_sample(inputs, grid, mode=mode, align_corners=True)
...
if __name__ == '__main__':
    size_fea = (1, 3, 160, 192, 160)
    A = torch.ones(size_fea)
    import tifffile as tif
    path1 = '/data/XiaolongWu/miccai24/data/lits_val/0/segmentation.tif'
    path2 = '/data/XiaolongWu/miccai24/data/lits_val/0/segmentation.tif'
    image1 = tif.imread(path1)[np.newaxis]
    image2 = tif.imread(path2)[np.newaxis]
    seg1 = torch.from_numpy(image1).type(torch.float32)
    seg2 = torch.from_numpy(image2).type(torch.float32)
    dsc = mask_metrics(seg1, seg2)
    # jd = jacobian_determinant_vxm(A.squeeze())
    print(dsc)
